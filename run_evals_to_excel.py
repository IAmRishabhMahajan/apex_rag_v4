...
def _run_hotpotqa_on(examples: list[dict]) -> dict[str, Any]:
    from benchmarks._utils import (avg, best_exact_match, best_token_f1,
                                   make_item, retrieved_ids, safe_run_pipeline)
    ...
    for i, ex in enumerate(examples):
        ...
        for title, sents in zip(context["title"], context["sentences"]):
            ...
            if content:
                items.append(make_item(
                    content=content, source_id=title, title=title,
                    url=f"https://en.wikipedia.org/wiki/{title.replace(' ','_')}",
                    expert="search", query=query,
                ))
        ...
        answer_ems.append(aem); answer_f1s.append(af1)
        sf_f1s.append(sff);     joint_f1s.append(jf1)
    ...
    return {
        "benchmark": "HotpotQA", "config": "distractor", "split": "validation",
        "evaluated": len(examples) - failures, "total": len(examples),
        "failures": failures, "elapsed_s": time.perf_counter() - t0,
        "answer_em": avg(answer_ems), "answer_f1": avg(answer_f1s),
        "sf_f1": avg(sf_f1s), "joint_f1": avg(joint_f1s),
...
def _run_nq_on(examples: list[dict]) -> dict[str, Any]:
    ...
    for i, ex in enumerate(examples):
        ...
        items = [make_item(content=passage, source_id=title, title=title,
                           url=url, expert="search", query=query)]
        ...
        if short_answers:
            ...
            short_ems.append(em); short_f1s.append(f1)
        elif yn_answers:
            q_type = "yes_no"
            yn_total += 1
            p = pred.strip().lower()
    ...
    return {
        "benchmark": "Natural Questions", "config": "standard", "split": "validation",
        "evaluated": len(examples) - failures, "total": len(examples),
        "failures": failures, "elapsed_s": time.perf_counter() - t0,
        "short_em": avg(short_ems), "short_f1": avg(short_f1s),
...
def _run_triviaqa_on(examples: list[dict]) -> dict[str, Any]:
    from benchmarks._utils import (avg, best_exact_match, best_token_f1,
                                   compute_recall_at_k, make_item,
                                   retrieved_ids, safe_run_pipeline)
    ...
    for i, ex in enumerate(examples):
        ...
        items = [make_item(content=it["content"], source_id=it["source_id"],
                           title=it["title"], url=it["url"],
                           expert="search", query=query)
                 for it in items_data if it.get("content", "").strip()]
        if not items:
            failures += 1
            continue
    ...
    return {
        "benchmark": "TriviaQA", "config": "rc", "split": "validation",
        "evaluated": len(examples) - failures, "total": len(examples),
        "failures": failures, "elapsed_s": time.perf_counter() - t0,
        "em": avg(ems), "f1": avg(f1s), "retrieval_recall": avg(recalls),
    }
...
def _run_ragbench_subset_on(subset_name: str, examples: list[dict]) -> dict[str, Any]:
    ...
    for i, ex in enumerate(examples):
        ...
        items = [make_item(content=d, source_id=f"ragbench-{subset_name}-{j}",
                           title=f"Document {j+1}", url="", expert="search", query=query)
...
def run_ragbench(max_examples: int, refresh: bool, subsets: list[str] | None = None) -> dict[str, Any]:
    from benchmarks._utils import avg
    ...
    return {
        "benchmark": "RAGBench", "config": f"{len(subsets)} subsets", "split": "train",
        "evaluated": total_examples - failures, "total": total_examples,
        "failures": failures, "elapsed_s": 0.0,
...
def _run_multihop_on(examples: list[dict]) -> dict[str, Any]:
    from benchmarks._utils import (avg, best_exact_match, best_token_f1,
                                   compute_mrr, compute_precision_at_k,
                                   compute_recall_at_k, make_item,
                                   retrieved_ids, safe_run_pipeline)
    ...
    for i, ex in enumerate(examples):
        ...
        items = [make_item(content=ev["content"], source_id=ev["source_id"],
                           title=ev.get("title", ""), url=ev.get("url", ""),
                           expert="search", query=query)
                 for ev in evidence if ev.get("content", "").strip()]
        if not items:
            failures += 1
            continue
    ...
    return {
        "benchmark": "MultiHop-RAG", "config": f"k={K}", "split": "train",
        "evaluated": len(examples) - failures, "total": len(examples),
        "failures": failures, "elapsed_s": time.perf_counter() - t0,
        "mrr": avg(mrrs), "recall_at_k": avg(recalls), "precision_at_k": avg(precisions),
        "answer_em": avg(ems), "answer_f1": avg(f1s),
...
def _sheet_summary(wb, results: list[dict], run_ts: datetime, total_s: float):
    ...
    c = ws["A2"]
    c.value = (f"Run: {run_ts.strftime('%Y-%m-%d  %H:%M:%S')}   |   "
               f"Total elapsed: {total_s:.1f}s")
    c.font  = Font(italic=True, size=10, color="595959")
    c.alignment = Alignment(horizontal="center", vertical="center")
    ws.row_dimensions[2].height = 18

    HEADERS = ["Benchmark", "Config", "Evaluated", "Failures",
               "Primary Metric", "Score", "Secondary Metric", "Score"]
    ...
    for d in results:
        ...
        _cell(ws, r, 4, d["failures"],      align="center",
              fill=RED if d["failures"] > 0 else GREEN)
...
def _sheet_hotpotqa(wb, d: dict):
    ws = wb.create_sheet("HotpotQA"); ws.sheet_view.showGridLines = False
    ws.merge_cells("A1:D1"); ws["A1"].value = f"HotpotQA [{d['config']}]  -  {d['split']} split"
    ws["A1"].font = Font(bold=True, size=13, color="1F3864")
    ws["A1"].alignment = Alignment(horizontal="center", vertical="center")
    ws.row_dimensions[1].height = 28
    metrics = [("Examples evaluated", d["evaluated"]), ("Failures", d["failures"]),
               ("Answer EM", d["answer_em"]), ("Answer F1", d["answer_f1"]),
               ("Supporting-Fact F1", d["sf_f1"]), ("Joint F1", d["joint_f1"]),
    ...
    for i, (lbl, val) in enumerate(metrics, 4):
        ...
        if isinstance(val, float) and "Elapsed" not in lbl: c.number_format = "0.000"
    r = 4 + len(metrics) + 2
    for bkt_name, bkt in [("By Question Type", d.get("by_type", {})),
                           ("By Difficulty Level", d.get("by_level", {}))]:
        if not bkt: continue
        ws.cell(row=r, column=1, value=bkt_name).font = BOLD; r += 1
        _header(ws, r, [bkt_name.split()[-1], "Answer F1", "SF F1", "Joint F1"]); r += 1
        for key, scores in bkt.items():
            ...
            for ci, m in enumerate(["answer_f1", "sf_f1", "joint_f1"], 2):
                ...
                c = _cell(ws, r, ci, round(v, 4), fill=_score_fill(v)); c.number_format = "0.000"
            r += 1
        r += 1
    _auto_width(ws)
...
def _sheet_nq(wb, d: dict):
    ws = wb.create_sheet("Natural Questions"); ws.sheet_view.showGridLines = False
    ws.merge_cells("A1:D1"); ws["A1"].value = f"Natural Questions  -  {d['split']} split"
    ws["A1"].font = Font(bold=True, size=13, color="1F3864")
    ws["A1"].alignment = Alignment(horizontal="center", vertical="center")
    ws.row_dimensions[1].height = 28
    metrics = [("Examples evaluated", d["evaluated"]), ("Failures", d["failures"]),
               ("Short-Answer EM", d["short_em"]), ("Short-Answer F1", d["short_f1"]),
    ...
    r = 4 + len(metrics) + 2
    if d.get("by_type"):
        ws.cell(row=r, column=1, value="By Answer Type").font = BOLD; r += 1
        type_keys = list(next(iter(d["by_type"].values())).keys())
        _header(ws, r, ["Answer Type"] + type_keys); r += 1
...
def _sheet_triviaqa(wb, d: dict):
    ws = wb.create_sheet("TriviaQA"); ws.sheet_view.showGridLines = False
    ws.merge_cells("A1:D1"); ws["A1"].value = f"TriviaQA [{d['config']}]  -  {d['split']} split"
    ws["A1"].font = Font(bold=True, size=13, color="1F3864")
    ws["A1"].alignment = Alignment(horizontal="center", vertical="center")
    ws.row_dimensions[1].height = 28
    metrics = [("Examples evaluated", d["evaluated"]), ("Failures", d["failures"]),
               ("EM (alias-matched)", d["em"]), ("F1 (alias-matched)", d["f1"]),
...
def _sheet_ragbench(wb, d: dict):
    ws = wb.create_sheet("RAGBench"); ws.sheet_view.showGridLines = False
    ws.merge_cells("A1:F1"); ws["A1"].value = f"RAGBench [{d['config']}]  -  {d['split']} split"
    ws["A1"].font = Font(bold=True, size=13, color="1F3864")
    ws["A1"].alignment = Alignment(horizontal="center", vertical="center")
    ws.row_dimensions[1].height = 28
    metrics = [("Examples evaluated", d["evaluated"]), ("Failures", d["failures"]),
    ...
    r = 4 + len(metrics) + 2
    if d.get("by_subset"):
        ws.cell(row=r, column=1, value="By Domain Subset").font = BOLD; r += 1
        _header(ws, r, ["Subset", "Evaluated", "Failures", "Adherence", "Claim Support", "Has Limitations"]); r += 1
        for name, sd in d["by_subset"].items():
            _cell(ws, r, 1, name, align="left", bold=True)
            _cell(ws, r, 2, sd["evaluated"], align="center")
            _cell(ws, r, 3, sd["failures"],  align="center",
                  fill=RED if sd["failures"] > 0 else GREEN)
...
def _sheet_multihop(wb, d: dict):
    ws = wb.create_sheet("MultiHop-RAG"); ws.sheet_view.showGridLines = False
    ws.merge_cells("A1:F1"); ws["A1"].value = f"MultiHop-RAG [{d['config']}]  -  {d['split']} split"
    ws["A1"].font = Font(bold=True, size=13, color="1F3864")
    ws["A1"].alignment = Alignment(horizontal="center", vertical="center")
    ws.row_dimensions[1].height = 28
    metrics = [("Examples evaluated", d["evaluated"]), ("Failures", d["failures"]),
               ("MRR@5", d["mrr"]), ("Recall@5", d["recall_at_k"]),
               ("Precision@5", d["precision_at_k"]), ("Answer EM", d["answer_em"]),
               ("Answer F1", d["answer_f1"]), ("Null Handling Rate", d["null_handling_rate"]),
               ("Elapsed (s)", round(d["elapsed_s"], 2))]
    _header(ws, 3, ["Metric", "Value"])
    for i, (lbl, val) in enumerate(metrics, 4):
        _cell(ws, i, 1, lbl, bold=True, align="left", fill=LGREY)
    ...
    r = 4 + len(metrics) + 2
    if d.get("by_type"):
        ws.cell(row=r, column=1, value="By Question Type").font = BOLD; r += 1
        type_keys = list(next(iter(d["by_type"].values())).keys())
        _header(ws, r, ["Question Type"] + type_keys); r += 1
        for qtype, scores in d["by_type"].items():
            _cell(ws, r, 1, qtype, align="left")
            for ci, key in enumerate(type_keys, 2):
                v = float(scores.get(key) or 0)
...
def _save_excel(results: list[dict], out: Path, run_ts: datetime, total_s: float):
    ...
    for d in results:
        b = d["benchmark"]
        if b == "HotpotQA":            _sheet_hotpotqa(wb, d)
        elif b == "Natural Questions":  _sheet_nq(wb, d)
        elif b == "TriviaQA":           _sheet_triviaqa(wb, d)
        elif b == "RAGBench":           _sheet_ragbench(wb, d)
        elif b == "MultiHop-RAG":       _sheet_multihop(wb, d)
    wb.save(out)
...
def main():
    ...
    parser.add_argument("--benchmark", choices=[*RUNNERS.keys(), "all"], default="all",
                        help="Which benchmark to run (default: all)")
    parser.add_argument("--max-examples", type=int, default=100, dest="max_examples",
                        help="Examples per benchmark (default: 100)")
    parser.add_argument("--out", default="eval_results.xlsx",
                        help="Output Excel file (default: eval_results.xlsx)")
    parser.add_argument("--refresh", action="store_true",
                        help="Re-download datasets even if local cache exists")
    ...
    for idx, name in enumerate(targets, 1):
        ...
        try:
            ...
            results.append(d)
            print(f"   done  ({d['evaluated']} evaluated, {d['failures']} failures, "
                  f"{d['elapsed_s']:.1f}s)", flush=True)

            # Save Excel incrementally after each benchmark completes
            _save_excel(results, out, run_ts, time.perf_counter() - t0)
            print(f"   Excel updated -> {out.name}  ({len(results)}/{len(targets)} benchmarks)", flush=True)
